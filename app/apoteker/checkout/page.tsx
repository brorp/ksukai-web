...
export default function CheckoutPage() {
        ...
        if (detail) {
          ...
          setSelectedPaymentMethod(
            detail.payment_method &&
              ALLOWED_PAYMENT_METHODS.has(detail.payment_method)
              ? detail.payment_method
              : "gopay",
          );
        }
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(async () => {
      try {
        const detail = await transactionApi.syncStatus(
          token,
  ...
  const redirectToPaymentPage = (transaction: Transaction) => {
    const paymentPageUrl =
      transaction.payment_page_url ?? transaction.payment_gateway_url;
    ...
    setError(
      "Halaman pembayaran belum tersedia untuk order ini. Silakan coba lagi.",
    );
  };
  ...
  const normalizedCurrentPaymentMethod =
    ...
    ALLOWED_PAYMENT_METHODS.has(currentTransaction.payment_method)
      ? currentTransaction.payment_method
      : null;
  ...
  const canPay =
    ...
    PENDING_STATUSES.has(currentTransaction.status) &&
    !!(
      currentTransaction.payment_page_url ??
      currentTransaction.payment_gateway_url
    );
  ...
  const orderActionLabel = !currentTransaction
  ...
  const canContinueExistingPayment =
    canPay && normalizedCurrentPaymentMethod === selectedPaymentMethod;
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-slate-500">
            Ringkasan order dan langkah pembayaran paket Anda.
          </p>
        </div>
        ...
        <Card className="border border-slate-200">
          <CardContent className="space-y-4 py-16 text-center">
            ...
            <p className="text-sm text-slate-500">
              Buka dashboard dan pilih paket untuk memulai checkout.
            </p>
          ...
          <div className="space-y-6">
            <Card className="border border-slate-200">
              <CardHeader>
                ...
                <CardDescription>
                  Detail order yang akan dibayarkan.
                </CardDescription>
              </CardHeader>
            ...
            {shouldShowOrderAction ? (
              <div className="space-y-3">
                ...
                normalizedCurrentPaymentMethod !== selectedPaymentMethod ? (
                  <p className="text-sm text-slate-500">
                    Klik tombol di atas untuk membuat ulang halaman pembayaran
              ...
              <CardHeader>
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Order ID: {currentTransaction?.order_code ?? "-"}
                    </CardDescription>
                  </div>
                  <span className={getStatusTone(currentTransaction?.status)}>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                  <InfoBox
                    ...
                    value={formatCurrency(
                      currentTransaction?.gross_amount ?? selectedPackage.price,
                    )}
                  />
                  <InfoBox
                    label="Metode"
                  ...
                  <InfoBox
                    ...
                    value={currentTransaction?.payment_type ?? "-"}
                  />
                  <InfoBox
                    label="Status Pembayaran"
                    value={
                      formatPaymentStatusDetail(
                        currentTransaction?.payment_status_detail,
                      ) || getStatusLabel(currentTransaction?.status)
                    }
                  />
                  <InfoBox
                  ...
                  <InfoBox
                    label="Paid At"
                    value={formatDate(currentTransaction?.paid_at)}
                  />
                </div>
                {currentTransaction ? (
                  <div className="flex flex-wrap gap-3">
                    {canContinueExistingPayment ? (
                      <Button
                        onClick={() =>
                          redirectToPaymentPage(currentTransaction)
                        }
                      >
                        <CreditCard className="mr-2 h-4 w-4" />
                ...
                {currentTransaction?.status === "paid" ? (
                  <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm text-emerald-700">
                    ...
                    <p className="mt-2">
                      Akses paket diaktifkan hanya untuk paket yang dibeli pada
                      order ini.
                    </p>
                  </div>
                ) : null}
